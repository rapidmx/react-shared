...
- The **Project Info** containing the version of this package, which consumer you hit it in
  (`rapidmx/server`'s SSR pages, `web-client`, or `electron-client`), and your Node.js version
...
```
Steps to Reproduce:
1. Call `configureApiBaseUrl("https://mail.example.com")` then `apiFetch("/mailboxes")`
2. Inspect the outgoing request in the browser's network tab

Repro Rate: 3/3

Expected:
A cross-origin GET to `https://mail.example.com/api/mailboxes` with credentials included

Actual:
A same-origin relative request to `/api/mailboxes` is sent instead
// Insert full stack trace / network tab screenshot

Severity: HIGH

Project Info:
@rapidmx/react-shared: v0.1.0
Consumer: electron-client
NodeJS: 24.0
```
...
```
As a developer I would like `giphyApi.ts` to expose a way to cancel an in-flight search request,
so a fast typist's older search results can't race ahead of their newest keystrokes.
```
